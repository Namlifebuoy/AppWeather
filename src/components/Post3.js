...
import TabFunctions2 from './TabFunctions2';
...
const Post3 = ({item, index, setShowImage, setImg}) => {
      ...
      return (
        <View style={styles.boxNoti}>
          <Text style={styles.txtNoti}>
            Bài viết của bạn rất tiếc đã bị từ chối duyệt !
          </Text>
          <Text>Người kiểm duyệt của chúng tôi cho rằng : {item?.note}</Text>
        </View>
      );
    if (item?.status == 0)
      return (
        <View style={[styles.boxNoti, {backgroundColor: '#e5fdde'}]}>
          <Text style={styles.txtNoti}>Bài viết của bạn đang chờ duyệt !</Text>
        </View>
      );
    else return <></>;
  };
...
export default React.memo(Post3);
